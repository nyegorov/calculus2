...
inline void CCtrl<WTL::CEdit>::CtrlPaint(HDC hdc, RECT& /*rCtrl*/, RECT& rPaint)
{
	CDCHandle(hdc).FillSolidRect(&rPaint, GetSysColor(COLOR_WINDOW));
	DefCtrlPaint(hdc, rPaint);
};
	...
	for (int i = 0; i < nBtn; i++)
	{
		...
		if (part != TP_SEPARATOR)
		{
			...
			IMAGELISTDRAWPARAMS ildp = {sizeof(IMAGELISTDRAWPARAMS), img, tbbi.iImage, hdc, x, y, 0, 0, 0, 0,
				CLR_NONE, CLR_NONE, ILD_PRESERVEALPHA, SRCCOPY, (state & TS_DISABLED)? (DWORD)ILS_SATURATE : ILS_NORMAL};
